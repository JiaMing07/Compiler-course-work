...
class CFG:
    def __init__(self, nodes: list[BasicBlock], edges: list[(int, int)]) -> None:
        ...
        """
        self.reachable_blocks = set()
        q = Queue()
        q.put(0)
        while not q.empty():
            visited_node = q.get()
            self.reachable_blocks.add(visited_node)
            for n in self.links[visited_node][1]:
                if n not in self.reachable_blocks:
                    q.put(n)
    ...
    def iterator(self):
        for n in self.reachable_blocks:
            yield self.nodes[n]
